...
Về bản chất, từ khóa `extends` sử dụng cơ chế [kế thừa prototype](/native-prototypes-trong-javascript/) thông qua việc gán `Rabbit.prototype.[[Prototype]]` bằng `Animal.prototype`. Nghĩa là nếu một phương thức không được tìm thấy trong `Rabbit.animal` thì JavaScript sẽ tự động tìm kiếm trong `Animal.prototype`.
...
Ví dụ [một hàm trả về một class](/cu-phap-class-trong-javascript-co-ban/) như sau:

    function f(message) {  return class {    sayHi() {
          console.log(message);
        }
      };
    }
...
Ví dụ, class `Rabbit` kế thừa class `Animal` và ghi đè phương thức `stop`:
    ...
    class Rabbit extends Animal {
      ...
      stop() {    super.stop(); // gọi đến phương thức stop của Animal    this.hide(); // và rồi gọi phương thức hide của chính nó.  }}
...
Với cách làm trên thì đã có lỗi xảy ra. Và lỗi này được hiểu là:

    Phải gọi hàm khởi tạo của class cha trước khi truy cập vào `this`
    hoặc kết thúc hàm khởi tạo.
...
Vì vậy, mình sẽ sửa lại đoạn code trên như sau:
    ...
    class Rabbit extends Animal {
      constructor(name, earLength) {
        super(name);    this.earLength = earLength;
      }
      // ...
    }
